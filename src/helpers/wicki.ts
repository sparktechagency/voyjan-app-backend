...
import { IAddress, LocationInfo } from "../app/modules/address/address.interface";
...
export const savedLocationsInDB = async (locations: LocationInfo[],placeI?:string) => {
    ...
    for (const location of locations) {
       try {
        ...
        const [translate,category]=await Promise.all([
            translateLanguages(summary.extract,location.title,await getGetCategory(page),location.title),
            getGetCategory(page)
        ])
...
for (const category of categoriesDb) {
  if (categories.some((c:any) => c.toLowerCase().includes(category.name.toLowerCase()))) {
...
export const savedLocationsInDBParrelal = async (locations: LocationInfo[],placeI?:string) => {
...
export const addLanguagesInExistingAddress = async (address:IAddress & {_id:string}) => {
  try {


    const diff_lang = await translateLanguages(address.summary!, address.name,address.type!,address.formattedAddress)
...
export const addShortDescription = async (address:IAddress & {_id:string}) => {
  try {
    ...
    const [shortDescription,longDescription] = await Promise.all([generateAiContnents(address.summary! || address.name,200),generateAiContnents(address.summary!||address.name)]);
    


  
    
    const data = await Address.findOneAndUpdate({_id:address._id},{summary:shortDescription,long_descreption:longDescription}, { new: true });
...
export const addNotFoundData = async (lat:number,lon:number,radius = 10000 ) => {
...
export const addTypeInExistingAddress = async (address:IAddress & {_id:string}) => {
  try {
